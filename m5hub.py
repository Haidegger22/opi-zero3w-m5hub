...
class Hub:
    def __init__(self):
        ...
        # Перенос указателя в центр экрана у края (чтобы движение не «упиралось в
        # стенку») разрешён ТОЛЬКО когда существует этот файл. Ставит его сторож
        ...
        self.SNAP_DIRS = 4    # Артём выбрал 4 стороны (24.09.2026): диагонали убрали,
                              ...
                              # слегка косое нажатие давало диагональ вниз-вправо
        self.DIR_HYST = 5     # угловой гистерезис (градусы): Артём выбрал 5 —
                              # 10 было «липко» (нужно 2-3 смены направления),
                              # 0 дёргало. Медианный фильтр сглаживает сам (24.09.2026)
    ...
    def _j(self):
        ...
        # Смена доминирующей оси стика — сбрасываем ОБЕ истории и память направления.
        # Иначе после движения в бок «липкое» направление снапа держит старую ось:
        # по вертикали история сбрасывается (там большой сигнал), а по горизонтали —
        # нет (там только шум < 4000), и вертикаль не работает, пока снова не двинешь
        # в бок. Диагноз 24.09.2026: указатель стоял 4.8 с при удержании стика вверх.
        _dom = 'x' if abs(dx) >= abs(dy) else 'y'
        if getattr(self, '_dom', None) is None:
            self._dom = _dom
        elif _dom != self._dom and max(abs(dx), abs(dy)) > 4000:
            self._dom = _dom
            self._dx_hist.clear()
            self._dy_hist.clear()
            self._dir = None
        ...
        ux=self._vx+sx
        uy=self._vy+sy
        if (ux<0 or ux>self.sw-1 or uy<0 or uy>self.sh-1) \
                and os.path.exists(self.RECENTER_FLAG):
            # Указатель упёрся в край экрана. Дальше двигать нечего: прыжков нет,
            # а игровой мост «джойстик → клавиши» работает именно по прыжкам
            # указателя — движение в эту сторону пропадало совсем (Артём:
            # «вниз не идёт, как в стенку упёрся», 24.09.2026).
            # Лечим переносом в центр экрана. Перенос служебный: мост игнорирует
            # прыжки больше RESET_PX (см. joy-to-keys.py), поэтому курсор игры
            # от него не дёргается.
            self._vx=self.sw/2.0
            self._vy=self.sh/2.0
            ux=self._vx+sx
            uy=self._vy+sy
        # Целевая позиция
    ...
    def cleanup(self):
        # Отпустить клавиши CardKB, которые драйвер держит прямо сейчас: если его
        # останавливают посреди удержания, отпускание не уходит, и X-сервер считает
        # клавишу зажатой, повторяя её сам ~30 раз в секунду — в приложении это
        # выглядит как бесконечная прокрутка. Поймано на практике 24.09.2026
        # (зажатый KP_Down у игрового моста); снимается только явным KeyRelease.
        try:
            n = 0
            for i in sorted(list(self._kdown)):
                k = self._kdown.pop(i)
                if k in CKM:
                    self._kv(CKM[k], 0, k)
                    n += 1
